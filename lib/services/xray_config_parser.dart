...
/// с outbound-ом `proxy`. Конвертирует Xray-outbound в sing-box-outbound
/// и кладёт его в [NodeConfig.rawOutbound], чтобы ConfigBuilder использовал
/// его как есть.
///
...
NodeConfig? _nodeFromXrayConfig(Map<String, dynamic> cfg) {
  ...
  final converted = _convertOutbound(proxy);
  if (converted == null) return null;

  return NodeConfig(
    name: (name?.isNotEmpty == true) ? name! : (converted['server'] as String),
    protocol: _protocolFor(converted['type'] as String?),
    host: converted['server'] as String,
    port: (converted['server_port'] as num).toInt(),
    params: const {},
    rawOutbound: converted,
  );
}
...
Map<String, dynamic>? _convertOutbound(Map<String, dynamic> o) {
...
Map<String, dynamic>? _vless(
  ...
  final user = (vnext['users'] as List?)?.whereType<Map>().firstOrNull
      ?.cast<String, dynamic>();
...
Map<String, dynamic>? _trojan(
    Map<String, dynamic> settings, Map<String, dynamic> stream) {
  final srv = (settings['servers'] as List?)?.whereType<Map>().firstOrNull
      ?.cast<String, dynamic>();
...
Map<String, dynamic> _tls(Map<String, dynamic> stream,
  ...
  final reality =
      (stream['realitySettings'] as Map?)?.cast<String, dynamic>();
